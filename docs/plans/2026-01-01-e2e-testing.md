...
**IMPORTANT:** Use the `run_e2e_tests.sh` script which manages server lifecycle automatically.
...
This script:
- Starts the real ios_server.py
- Sets up transcript paths
- Runs all E2E tests
- Cleans up server and temp files

Expected: All tests run (may have failures to debug)

Note: Do NOT run tests directly with xcodebuild - the tests require the server to be running, which the script handles.
...
**IMPORTANT:** Always use the `run_e2e_tests.sh` script to run E2E tests. This script manages the server lifecycle automatically.
...
The script:
- Starts ios_server.py with test mode enabled
- Creates temp transcript file
- Runs all E2E UI tests
...
- Xcode 16+
- iPhone 16 Simulator (or edit script for your simulator)
- Python 3.9+ with venv at `../../.venv`
- Server script at `voice_server/ios_server.py`

## Troubleshooting

If tests fail:
1. Check server logs: `cat /tmp/e2e_server.log`
2. Verify server started: Check for "Server started successfully" message
3. Check transcript path: `/tmp/claude_voice_e2e_tests/transcript_*.jsonl`
```
