"""Tests for TTS queue behavior in ConnectServer."""
import pytest
import asyncio
import json
...
async def test_tts_queue_drains_to_latest(server):
    ...
    with patch('server.services.tts_manager.generate_tts_audio', side_effect=fake_generate), \
         patch('server.services.tts_manager.samples_to_wav_bytes', return_value=b"fake_wav"):
        # Start worker
