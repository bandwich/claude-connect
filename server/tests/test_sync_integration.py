...
from server.main import TranscriptHandler
...
class TestDeliveryVerification:
    ...
    async def test_verify_delivery_finds_user_message(self, tmp_path):
        ...
        with patch.object(ConnectServer, '__init__', lambda self: None):
...
class TestResyncHandler:
    ...
    async def test_resync_replays_from_sequence(self, tmp_path):
        ...
        # Patch ConnectServer.__init__ to avoid side effects (tmux, http server, etc.)
        with patch.object(ConnectServer, '__init__', lambda self: None):
            server = ConnectServer()
