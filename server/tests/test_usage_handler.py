# server/tests/test_usage_handler.py
import pytest
import json
import asyncio
import time
...
async def test_check_usage_calls_api():
    ...
    with patch("server.services.usage_checker.subprocess.run") as mock_run, \
         patch("server.services.usage_checker.aiohttp.ClientSession", return_value=mock_session):
